...
def corners_heuristic(node: CornersSearchNode, problem: CornersSearchProblem, **kwargs: typing.Any) -> float:
    ...
    for corner in problem.corners:
        if corner not in node.visited_corners:
            unvisited_corners.append(corner)
    
    # If no corners left, done
    if not unvisited_corners:
        return 0.0
    
    # Return distance to farthest unvisited corner
    return float(max(
...
def food_heuristic(node: pacai.search.food.FoodSearchNode, problem: pacai.search.food.FoodSearchProblem, **kwargs: typing.Any) -> float:
    ...
    # Convert remaining food to list
    remaining_food_list = list(node.remaining_food)
    
    # If there is one more food left, calculate Manhattan distance
    if len(remaining_food_list) == 1:
        food_pos = remaining_food_list[0]
        manhattan_dist = abs(node.position.row - food_pos.row) + abs(node.position.col - food_pos.col)
        return float(manhattan_dist)
    
    # Compute Manhattan distance for every food and takes min
    min_dist_to_food = min(
        abs(node.position.row - food.row) + abs(node.position.col - food.col)
        for food in remaining_food_list
    )
    
    n = len(remaining_food_list)
    dists = {}
    for i in range(n):
        for j in range(n):
            if i != j:
                manhattan_dist = (abs(remaining_food_list[i].row - remaining_food_list[j].row)
                                  + abs(remaining_food_list[i].col - remaining_food_list[j].col))
                dists[(i, j)] = manhattan_dist
    
    # MST setup (Prim's algorithm)
    cost = 0.0
    included = {0}
    remaining_indices = set(range(1, n))
    
    # Find cheapest edge connecting node in included to node in remaining indices
    while remaining_indices:
        min_edge_dist = float('inf')
        min_edge_to = None
        # Scans all valid connecting edges and tracks cheapest one
        for i in included:
            for j in remaining_indices:
                if (i, j) in dists:
                    if dists[(i, j)] < min_edge_dist:
                        min_edge_dist = dists[(i, j)]
                        min_edge_to = j
                elif (j, i) in dists:
                    if dists[(j, i)] < min_edge_dist:
                        min_edge_dist = dists[(j, i)]
                        min_edge_to = j
        if min_edge_to is not None:
            cost += min_edge_dist
            # which food to add next
            included.add(min_edge_to)
            remaining_indices.remove(min_edge_to)
        else:
            break

    # Fallback estimate (for safety)
    if remaining_indices:
        min_pairwise = float('inf')

        for i in range(len(remaining_food_list)):
            food1 = remaining_food_list[i]
            for j in range(i + 1, len(remaining_food_list)):
                food2 = remaining_food_list[j]
                dist = abs(food1.row - food2.row) + abs(food1.col - food2.col)
                if dist < min_pairwise:
                    min_pairwise = dist

        cost = (n - 1) * min_pairwise
    
    # Return total estimate = distance to nearest food + MST cost
    # lower bound
    return float(min_dist_to_food + cost)
